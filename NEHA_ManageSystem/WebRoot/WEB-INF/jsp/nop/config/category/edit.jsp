	...
	<form method="post" action="<s:property value="absolutePath" />npo/config/category/save.do" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);">
		...
		<div class="pageFormContent" layoutH="58">
			<div class="unit">
				...
				<input name="cateEvent.cgeId" class="required number" max="3999" min="3001" size="30" type="text" disabled="disabled" value='<s:property value="cateEvent.cgeId" />' />
				<span class="info"></span>
			</div>
			<div class="unit">
				<label><s:text name="npo.config.category.name"/>: </label>
				<input name="cateEvent.cgeName" class="required" maxlength="50" size="30" type="text" value='<s:property value="cateEvent.cgeName" />' />
				<span class="info"></span>
			</div>
			<div class="unit">
				<label><s:text name="npo.config.category.order"/>: </label>
				<input name="cateEvent.cgeOrder" class="required number" value='<s:property value="cateEvent.cgeOrder" />' max="9999" min="1" size="30" type="text" />
				<span class="info"></span>
			</div>
			<div class="unit">
				<label><s:text name="npo.config.category.status"/>: </label>
				<s:select list="#{'true':'\u6b63\u5e38','false':'\u505c\u7528'}" name="cateEvent.cgeStatus" theme="simple" cssClass="required combox"/>
				<span class="info"></span>
			</div>
			<div class="unit">
				<label><s:text name="npo.config.category.comment"/>: </label>
				<input name="cateEvent.cgeComment" class="" maxlength="500" size="30" type="text" value='<s:property value="cateEvent.cgeComment" />' />
				<span class="info"></span>
			</div>
